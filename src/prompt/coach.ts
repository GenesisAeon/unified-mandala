export type PromptFinding = {
  type: 'conflict' | 'missing_role' | 'vagueness';
  where: string;
  note: string;
};
...
export function heuristicOptimize(raw: string): PromptAdvice {
  ...
  if (/sei kurz/i.test(raw) && /ausführlich/i.test(raw)) {
    ...
    out = out.replace(/ausführlich/gi, 'prägnant aber vollständig');
  }
...
export async function maybeRemoteOptimize(
  raw: string,
  endpoint?: string,
  apiKey?: string,
): Promise<PromptAdvice | null> {
