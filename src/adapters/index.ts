...
const retry = <T>(next: Adapter<T>) => withRetry<T>({ retries: 3, backoffMs: 200, maxBackoffMs: 2000, jitter: true })(next);
...
export const era5 = withSchema<Era5Sample>(Era5Schema)(
  cache60s(rate(retry(era5Raw)))
);

export const oisst = withSchema<OisstSample>(OisstSchema)(
  cache60s(rate(retry(oisstRaw)))
);
