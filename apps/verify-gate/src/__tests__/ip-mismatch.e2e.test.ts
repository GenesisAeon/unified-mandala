...
describe('verify-gate metrics: IP mismatch', () => {
  beforeEach(() => {
    vi.resetModules();
    fetchMock.mockReset();
    ...
    process.env.VERIFY_GATE_SSRF_ALLOWLIST = 'https://example.com';
    process.env.VERIFY_UPSTREAM_URL = 'https://example.com';
  ...
  it('exposes verify_gate_ip_mismatch_total after proxy failure', async () => {
    ...
    expect(metricsResponse.text).toMatch(/verify_gate_ip_mismatch_total(?:\{[^}]+\})?\s+1(\.0+)?/);
  });
});
