...
- FR-UM-2025-11-10-Fraktal74-HuskyCIGates: Husky-Pre-Commit/-Pre-Push laufen über `pnpm check:precommit`/`pnpm check:prepush` (Bypass via `UM_SKIP_HEAVY_HOOKS=1`), neue Aliase `check:ci`/`ci:verify` rufen `scripts/ci-verify.mjs` auf und GitHub Workflows `check-ci.yml`/`ci-verify.yml` spiegeln die gebündelten Gates mit `PANTHEON_DISABLE=1`; `scripts/sanity/check-keys-empty.mjs` hält `keys/` leer, MandalaMap.\*, Stabilization-Playbook und Command-Catalog dokumentieren den Lauf; `scripts/sigils/mini-sigillin-validator.ts` erhielt einen CLI-Guard (argv-Check + typisierte stdin-Events), sodass `pnpm typecheck` wieder grün läuft.
