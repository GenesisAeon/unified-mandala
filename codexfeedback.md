...
- FR-UM-2025-10-19-Fraktal57-JetStream: JetStream-Bus-Test läuft unter Vitest; ci.core startet Docker-NATS und führt `pnpm nats:doctor` + `pnpm test:jetstream`; README/MandalaMap/Command-Catalog spiegeln den Flow; `scripts/dev-services.mjs` meldet Portkollisionen (`pnpm dev:ports:free`), `pnpm nats:docker` steht für lokale Läufe bereit, Emergence-Scan nutzt `docs/sigillin/GENESIS.md` als Fallback bei fehlender Root-Datei und die Adapter-Builds greifen via `scripts/lib/python.mjs`/`scripts/adapter-build-era5.mjs` auf die Projekt-venv (Windows-kompatibel).
