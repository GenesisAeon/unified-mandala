...
if (!fs.existsSync(base)) { console.log("No golden tests."); process.exit(0); }
const reg = Object.fromEntries(
  (loadRegistry()).map(r=>[r.id, r.entry])
);
let fails = 0;
for (const name of fs.readdirSync(base)) {
  ...
  if (!reg[agentId]) { console.warn(`[warn] Agent ${agentId} nicht registriert – übersprungen.`); continue; }
  const inputPath = path.join(dir, "input.json");
  const expectedPath = path.join(dir, "expected.json");
  if (!fs.existsSync(inputPath) || !fs.existsSync(expectedPath)) { console.warn(`[warn] fehlende input/expected in ${dir}`); continue; }
  const input = JSON.parse(fs.readFileSync(inputPath,"utf8"));
  const A = await loadAgentModule(reg[agentId]) as Agent;
  const out = await A.run(input, {
    logger: ()=>{},
    rateLimit: async()=>{},
    http: async()=>{ throw new Error("no-http-in-golden"); },
