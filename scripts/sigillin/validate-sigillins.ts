...
function hasAny(hay: string, needles: string[]) {
  ...
  return needles.some((k) => lower.includes(k.toLowerCase()));
}
...
async function validateOne(file: string): Promise<Result> {
  ...
  try {
    ...
    if (!data || typeof data !== 'object' || !('sigillin' in data)) {
      errors.push('Schema: /sigillin fehlt oder ist kein Objekt');
      return { file, valid: false, errors };
    }
