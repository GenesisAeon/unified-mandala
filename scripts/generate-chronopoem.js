...
try {
  const matrix = YAML.parse(
    fs.readFileSync(path.join(ROOT_DIR, 'docs', 'CREPPhaseMatrix.yaml'), 'utf8'),
  );
...
try {
  const files = fs.readdirSync(
    path.join(ROOT_DIR, 'packages/genesis-sigillin-core/schemas/examples/'),
  );
  sigils = files
...
const metaphorStr = Object.entries(crepMetaphern)
  .map(([k, v]) => `${k}=${v}`)
  .join(', ');

// Schreibe CHRONOPOEM.md
fs.writeFileSync(
  path.join(ROOT_DIR, 'CHRONOPOEM.md'),
  `# 🜂 Chronopoem
