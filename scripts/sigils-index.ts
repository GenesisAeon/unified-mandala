...
import { safeParseSigilFile } from './lib/safeSigilParse.js';
import { normalizeCREP } from '../src/domain/crep.js';
...
(async function main() {
  ...
  for (const file of files) {
    ...
    const id = (data.id ||
      data.slug ||
      path.basename(file).replace(path.extname(file), '')) as string;
    ...
    const crep =
      crepNorm.source !== 'none'
        ? { score: crepNorm.score, parts: crepNorm.parts, source: crepNorm.source }
        : undefined;
