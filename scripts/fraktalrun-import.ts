...
async function useAjv() {
  try {
    ...
    console.error(
      "✖ Benötige die Pakete 'ajv' und 'ajv-formats'. Bitte installieren:  npm i -D ajv ajv-formats",
    );
    process.exit(1);
  }
}
...
type Manifest = {
  ...
  meta?: AnyRec;
  sources: (
    | { path: string; format: 'yaml' | 'json' }
    | { optional: { path: string; format: 'yaml' | 'json' }[] }
  )[];
  schema: { task: string };
  mapping: string;
...
function normalizeTransforms(task: AnyRec) {
  ...
  if (Array.isArray(task.labels))
    task.labels = task.labels.map((l: any) => String(l).toLowerCase());
...
function dedupe(
  tasks: FraktalTask[],
  keys: string[],
  strategy: 'keep-highest-priority' | 'keep-first',
) {
