...
function convToText(conv) {
  ...
  const msgs = nodes.filter(
    (n) =>
      n.message &&
      n.message.content &&
      n.message.content.parts &&
      n.message.content.parts[0] !== '',
  );
  msgs.sort((a, b) => (a.message.create_time || 0) - (b.message.create_time || 0));
  return msgs
    .map((m) => `${m.message.author.role || 'unknown'}: ${m.message.content.parts.join('\n')}`)
    .join('\n');
}
...
const startIndex =
  fragments.length > 0
    ? parseInt(fragments[fragments.length - 1].id.match(/Fragment_(\d+)/)[1]) + 1
    : 1;
