...
describe('validateNewAdvancedConversations', () => {
  ...
  it('flags duplicate titles case-insensitively', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-duplicate-titles-case.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.duplicateTitles).toEqual(['foo']);
    expect(res.duplicateTitleIndices).toEqual([1]);
  });
  ...
  it('flags titles with extra whitespace and normalizes for duplicates', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-title-whitespace.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.duplicateTitles).toEqual(['foo']);
    expect(res.duplicateTitleIndices).toEqual([1]);
  ...
  it('flags titles with control characters', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-title-chars.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidTitleChars).toEqual([0]);
  });

  it('flags titles exceeding 100 characters', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-long-title.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithLongTitles).toEqual([0]);
  });

  it('flags nodes with invalid author roles', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-role.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidRoles).toEqual([0]);
  });

  it('flags nodes missing author roles', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-missing-role.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithMissingRoles).toEqual([0]);
  });

  it('flags nodes with invalid author names', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-author-name.json');
    const res = validateNewAdvancedConversations(file);
  ...
  it('flags nodes with invalid message timestamps', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-message-times.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidMessageTimestamps).toEqual([0]);
  });

  it('flags non-string message parts', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-part-types.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidContentParts).toEqual([0]);
  });

  it('flags invalid id formats', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-id-format.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidIds).toEqual([0]);
  });

  it('flags duplicate message ids', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-duplicate-message-id.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithDuplicateMessageIds).toEqual([0]);
  });

  it('flags invalid current_node references', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-current-node.json');
    const res = validateNewAdvancedConversations(file);
  ...
  it('flags invalid plugin and URL fields', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-extras.json');
    const res = validateNewAdvancedConversations(file);
  ...
  it('flags messages with weight outside 0-1', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-message-weight.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidMessageWeights).toEqual([0]);
  });

  it('flags messages with invalid status values', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-message-status.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidMessageStatuses).toEqual([0]);
  });

  it('flags messages with invalid channel values', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-message-channel.json');
    const res = validateNewAdvancedConversations(file);
  ...
  it('flags messages with invalid content_type values', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-content-type.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidContentTypes).toEqual([0]);
  });

  it('flags messages with invalid end_turn values', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-end-turn.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidEndTurn).toEqual([0]);
  });

  it('flags cyclic child references', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-cyclic-reference.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithCyclicReferences).toEqual([0]);
  });

  it('flags missing attachments', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-missing-attachment.json');
    const res = validateNewAdvancedConversations(file);
  ...
  it('flags invalid conversation_template_id values', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-template-id.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidTemplateId).toEqual([0]);
  });

  it('flags invalid default_model_slug values', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-model-slug.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidDefaultModelSlug).toEqual([0]);
  });

  it('flags invalid async_status values', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-async-status.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidAsyncStatus).toEqual([0]);
  });

  it('flags empty voice fields', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-voice.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidVoice).toEqual([0]);
  });

  it('flags invalid memory_scope values', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-memory-scope.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidMemoryScope).toEqual([0]);
  });

  it('flags invalid boolean flags', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-boolean-flags.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidBooleanFlags).toEqual([0]);
  });

  it('flags invalid gizmo metadata fields', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-gizmo-metadata.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidGizmoMetadata).toEqual([0]);
  });

  it('flags invalid origin fields', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-origin-fields.json');
    const res = validateNewAdvancedConversations(file);
  ...
  it('flags invalid blocked urls', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-blocked-urls.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidBlockedUrls).toEqual([0]);
  });
  ...
  it('flags invalid safe urls', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-safe-urls.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidSafeUrls).toEqual([0]);
  });

  it('flags safe urls with unsupported protocols', () => {
    const file = path.join(__dirname, '../tests/fixtures/newadvanced-invalid-safe-url-scheme.json');
    const res = validateNewAdvancedConversations(file);
    expect(res.conversationsWithInvalidSafeUrls).toEqual([0]);
  });
});
