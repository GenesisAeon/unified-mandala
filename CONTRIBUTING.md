...
- Vor Merge einmal `pnpm build:ui` + `pnpm dev:services` (Smoke `/` → 200) prüfen oder den Vite-Dev-Server via `pnpm dev` verifizieren.
...
- Entwicklungsmodus (Service-Orchestrator): `pnpm dev:stack` (nutzt `scripts/dev-services.mjs`).
- UI-Dist-Server: `pnpm dev:services` (nutzt `tsx`).
