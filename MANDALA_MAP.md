...
| `agents/personhood/AIJuristicAgent.ts` + `data/personhood/cases/` (2. Durchgang, 2026-07-26) | 76 Zeilen TS               | Einziger echter Ausreißer unter den 32 `agents/`-Unterordnern (siehe Blueprint-Tabelle unten für die anderen 29): eine selbstgeschriebene TF-IDF-Kosinus-Ähnlichkeitssuche klassifiziert eine "Rechtspersönlichkeits"-Anfrage (Fluss/Wald/Baum/KI/Organisation) gegen echte Präzedenzfall-Dokumente (`data/personhood/cases/`: `river_whanganui_nz.md`, `colombia_atria_river.md`, `eu_ai_act_discussion_stub.md`) und vergibt ein Schutzlevel P0/P1/P2 mit Konfidenzwert. Kein Stub, keine Platzhalter-Werte — echte, wenn auch kleine, funktionierende Logik.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Klein, aber eigenständig — Kandidat für P55, falls Johann diesen Bereich (KI-/Natur-Rechtspersönlichkeit) weiterverfolgen will.                                                                                                                                                                                                                                                                                                                                                                    |
...
| -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sims/`, `demos/`, `examples/`                                                                                                                     | Noch nicht inhaltlich gesichtet (2. Durchgang deckte nur `experiments/`, `agents/`, Go-Code ab)                                                                                                                                                                                                                                                                                      | Dritter Durchgang: gleiche Heuristik anwenden                                                                                                                                                  |
| `go-agent/` + `go-bridge/` (2. Durchgang, 2026-07-26: gesichtet, ~2354 Zeilen)                                                                     | Beide sind echte, aktive Infrastruktur (CLI `mandala-cli` mit `meta-scores`/`crep watch`-Kommandos, ein Scheduler-Service mit NATS/Vault/Metrics) — aber unklar, ob sie wie `src/`/`apps/`/`packages/` als "aktiver Code dieses Repos selbst" gelten sollen, oder ob eine eigene Go-Paket-Konvention (kein Äquivalent zu den Python-`aeon-*`-Paketen existiert bisher) sinnvoll wäre | Johann entscheidet: als Teil des Kernrepos belassen, oder eigene Release-Konvention für Go-Module aufsetzen?                                                                                   |
| `packages/{socialgoodmesh,userfriendship}`, `services/{chat-writer,vector-indexer}` (Go, 2. Durchgang, 2026-07-26: gesichtet, 467 Zeilen zusammen) | Reale, funktionierende, aber generische Mini-Services (In-Memory-CRUD, JSON-Message-Writer, Embedding-Indexer) ohne erkennbaren `UTAC`/`CREP`/`Gamma`/GenesisAeon-Bezug im Code                                                                                                                                                                                                      | Eigenständige Nebenprojekte, spätere Integration, oder Kandidaten zum Entfernen? Keine Empfehlung ohne Johanns Einschätzung, wofür sie gedacht waren                                           |
...
`examples/` bleiben ungesichtet für einen dritten Durchgang.
