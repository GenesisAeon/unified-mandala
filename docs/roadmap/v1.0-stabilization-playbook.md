...
- _Status 2025-11-18 (Fraktal77):_ `auto-provenance.yml` versieht PRs automatisch mit `source:*`-Labels, `provenance-gate.yml` blockt Pfad-Verletzungen je Label (Two-Plane + Protected Zones) und `ci.core` wurde in leichte + optionale Advisory-/Runtime-Lanes zerlegt (`on-demand.yml`, `ci.runtime.yml`, label-gesteuerte Checks). PR-Template, CODEOWNERS und `scripts/repo/setup-labels.mjs` dokumentieren den Pflicht-Hook; `docs/START-HERE.md` fasst Golden Path & Label-Matrix für Onboarding zusammen.
