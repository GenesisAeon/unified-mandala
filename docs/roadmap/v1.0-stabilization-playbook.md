...
- Unified-Mandala ist ein PNPM-Monorepo mit `apps/`, `packages/` und weitreichenden Skripten im Projektstamm (`package.json`). Die Core-Build-Pipeline nutzt `pnpm lint:eslint`, `pnpm format:check`, `pnpm test:ts:ci`, `pnpm test:py` und `npx pyright` innerhalb von `.github/workflows/ci.core.yml` (Node 20, Offline-/Low-Mem-Flags).【F:package.json†L12-L111】【F:.github/workflows/ci.core.yml†L1-L38】
- Das Experimental-Workflow (`ci.experimental.yml`) läuft fail-fast: Guardrails- und Agenten-Dry-Runs liefern Artefakte, brechen den Lauf bei Policy-Verstößen jedoch unmittelbar ab.【F:.github/workflows/ci.experimental.yml†L1-L40】
...
1. **Core-Gates härten**
   - `ci.core.yml` führt `pnpm lint:eslint`, `pnpm format:check`, `pnpm test:ts:ci`, `pnpm test:py` und `npx pyright` mit Fail-Fast-Semantik aus; eine identische Matrix steht für Nightly-Läufe bereit.
...
2. **Extended-/Experimental-Schicht**
   - In `ci.experimental.yml` die `|| true`-Konstrukte durch echte Fehlerpfade ersetzen; Agents-/Policy-Dry-Runs liefern weiterhin Artefakte, schlagen aber hart fehl.
   - `ci.extended.yml` ergänzt `pnpm test:ts:coverage` (Vitest Coverage) und lädt `resonance:smoke` Logs als Artefakt hoch; Fehler beenden den Lauf unmittelbar.
...
3. **Coverage & Reporting**
   - `ci.extended.yml` erzeugt Coverage via `pnpm test:ts:coverage`; weitere Auswertung (Badges, Thresholds) folgt nach Stabilisierung.
...
2. **Dokumentation**
   - `AI_POLICY.md` und `AI_POLICY.yaml` mit Allow/Deny-Beispielen und Guardrail→Fix-Matrix ergänzen.
   - Governance-Primer in `docs/CommunityOnboarding.md` verankern und auf `policies/merge-guardrails.yaml` referenzieren.
...
1. **README / CONTRIBUTING / ONBOARDING**
   ...
   - Quickstart um `scripts/setup-dev-env.sh`, `pnpm policy:check` und `pnpm start:light` ergänzen.
2. **AI Governance Doku**
   - `AI_POLICY.md` mit praktischen Beispielen, Guardrail-Handlungsanleitungen und Community-Primer hinterlegen.
...
- `observability/prometheus.yml` und `observability/grafana/` in Deploy-Pipeline einbinden; Compose-Profile um Prometheus/Grafana ergänzen (Profil `monitoring` in `docker-compose.yml`).【F:observability/prometheus.yml†L1-L7】
