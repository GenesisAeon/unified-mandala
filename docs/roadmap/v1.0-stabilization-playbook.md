...
- _Status 2025-10-19 (Fraktal57):_ JetStream-Bus-Test läuft unter Vitest, CI-Core startet einen Docker-basierten NATS-Server, ruft `pnpm nats:doctor` und `pnpm test:jetstream` als Teil des type-and-tests-Gates auf; lokale Workflows verwenden `pnpm nats:docker` für JetStream-Container und `pnpm dev:ports:free`/Port-Wächter im Dev-Stack, um Windows-Portkollisionen früh zu erkennen. Das Runbook `docs/runbooks/nats-jetstream.md` dokumentiert Setup, Diagnose und Troubleshooting (inkl. `$JS.API.INFO` Fallback).
