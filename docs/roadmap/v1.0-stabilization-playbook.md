...
## Ownership & aktueller Stand (Fraktal43)

| Stream        | Owner            | Fortschritt 2025-09-23                                                       |
| ------------- | ---------------- | ---------------------------------------------------------------------------- |
| stability     | `codex-core`     | Nightly-Härtung offen, Dist-first Agent CLI vorbereitet                      |
| code-quality  | `agents-guild`   | `ts-node` aus Produktionsskripten entfernt, README/CONTRIBUTING aktualisiert |
| build-release | `infra-circle`   | Docker/Compose unverändert, Smoke-Tests geplant                              |
| governance    | `ethics-lab`     | AI-Policy-Beispiele ergänzt, Guardrail-Handbuch in `AI_POLICY.md`            |
| observability | `monitoring-hub` | Compose-Profil aktiv, `/metrics`-Rollout noch geplant                        |

### Fraktal43 Umsetzungsschritte

- **Dist-first Agent CLI**: `pnpm agents:build` kompiliert `scripts/agents/*` nach `dist/`, `pnpm agents:run`/`agents:health` starten nur noch gebaute Artefakte (keine `ts-node`-Pfadabhängigkeit).
- **Governance-Dokumentation**: `AI_POLICY.md` enthält Allow/Review/Block-Beispiele sowie eine Erläuterung der Guardrail-IDs (`policies/merge-guardrails.yaml`). `AI_POLICY.yaml` wurde um einen Data-Steward-Blocker ergänzt.
- **Onboarding-Docs**: README & CONTRIBUTING verweisen auf die neue Dist-first-Nutzung der Agentenwerkzeuge.
- **Tracking**: Statusübernahme nach `codexfeedback.*` (inklusive `V1ready` Marker) und Aktualisierung der Playbook-Owner.
