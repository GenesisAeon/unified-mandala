...
- _Status 2025-11-10 (Fraktal74):_ Husky-Pre-Commit/-Pre-Push nutzen jetzt `pnpm check:precommit`/`pnpm check:prepush` (Typecheck, Unit, Schema/Maps, Repomap, Sanity, Policy bzw. Coverage + Policy) mit optionalem `UM_SKIP_HEAVY_HOOKS=1`-Bypass; neue PNPM-Aliase `check:ci`/`ci:verify` und die Workflows `check-ci.yml`/`ci-verify.yml` spiegeln die gebündelten Gates, `scripts/ci-verify.mjs` sowie `scripts/sanity/check-keys-empty.mjs` halten Repo-Sanity konsistent; Mini-Sigillin-Validator CLI-Guard behebt den TypeScript-Fehler und hält `pnpm typecheck` grün.
...
- _Status 2025-11-02 (Fraktal67):_ DevTalk67 ausgewertet; das Workflow-Cheat-Sheet ergänzt `pnpm check:ci`, `pnpm ci:verify`, Monitoring-/Offline-Bundle-Hinweise sowie den Windows-Setup-Pfad (`scripts/setup-dev-env.ps1`). Codexfeedback\* und MandalaMap\* spiegeln den erweiterten Scope.
