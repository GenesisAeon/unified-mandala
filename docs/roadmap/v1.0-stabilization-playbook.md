...
1. **Core-Gates härten**
   ...
   - _Status 2025-09-26:_ Format/Lint sind Teil des Core-Jobs, Toolchain-Logging + Artefakt aktiv; Nightly-Matrix spiegelt Core, Ergebnisse werden automatisiert ausgewertet und archiviert.
2. **Extended-/Experimental-Schicht**
   ...
   - _Status 2025-09-26:_ Experimental-Jobs laufen auf Node 20 und nutzen `agents:health`; Guardrail-/Agenten-Läufe schlagen nun ohne `|| true` fehl und Coverage-Reports sind stabil eingebunden.
...
2. **Dead Code & Skript-Konsolidierung**
   ...
   - _Status 2025-09-26:_ `scripts/run-dist.mjs` ersetzt `ts-node`, Dist-First-Richtlinien sind in README/CONTRIBUTING verankert und CLI-Dokumentation wurde abgeschlossen.
...
1. **Dist-First Strategie**
   ...
   - _Status 2025-09-26:_ Node20-Basiscontainer mit Go & Python aktiv; Dist-First-Kommandos laufen ausschließlich über `scripts/run-dist.mjs`, Produktions- und Dev-Umgebung sind synchronisiert.
2. **Runtime Smoke Tests**
   ...
   - _Status 2025-09-26:_ `pnpm smoke:light-static` nutzt den Light-Server (inkl. Health-Endpoint) für Brotli/Gzip-Checks; Workflow-Gates referenzieren die Smoke-Skripte direkt.
...
3. **Docker/Compose Integration**
   ...
   - _Status 2025-09-26:_ Standard-Aufruf (`docker compose up`) startet nur `ui`; Release-Drill (`pnpm build` → `pnpm start:light` → Compose + Monitoring) wurde erfolgreich getestet.
...
2. **Dokumentation**
   ...
   - _Status 2025-09-26:_ Entscheidungsbeispiele für Allow/Review/Block sind in Markdown und YAML dokumentiert; Governance-Primer und Response-Guidelines sind veröffentlicht.
...
- _Status 2025-09-26:_ Compose-Profil `monitoring` steht zur Verfügung (`docker compose --profile monitoring up`) und `/metrics`-Smoke-Checks laufen als Teil des Release-Drills.
