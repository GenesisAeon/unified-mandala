...
1. **Core-Gates härten**
   ...
   - _Status 2025-09-30:_ Core-Jobs laufen fail-fast, Nightly-Matrix spiegelt Core inkl. Toolchain-Artefakt; Fraktal44 dokumentiert die Auswertung & Alerts.
2. **Extended-/Experimental-Schicht**
   ...
   - _Status 2025-09-30:_ Experimental/Extended-Jobs auf Node 20, `agents:health` aktiv, Coverage-Job dokumentiert; Follow-ups in Fraktal44 abgeschlossen.
...
2. **Dead Code & Skript-Konsolidierung**
   ...
   - _Status 2025-09-30:_ `scripts/run-dist.mjs` ersetzt `ts-node`, README/CONTRIBUTING/ONBOARDING aktualisiert; neue Kommandos (`pnpm dev`, `pnpm dev:services`, `pnpm dev:stack`) dokumentiert.
...
1. **Dist-First Strategie**
   ...
   - _Status 2025-09-30:_ Node20-Basiscontainer aktiv, Dist-First-Kommandos via `scripts/run-dist.mjs`; Compose/Docker-Kommandos auf neue Dev-Skripte abgestimmt.
2. **Runtime Smoke Tests**
   ...
   - _Status 2025-09-30:_ `pnpm smoke:light-static` aktiv, Sigillin-Validator ergänzt; Smoke-Skripte über Fraktal44 in CI-Dokumentation verankert.
...
- _Status 2025-09-30:_ Compose-Profil `monitoring` produktiv; `/metrics`-Rollout dokumentiert, Smoke-Checks in Release-Drill verankert.
