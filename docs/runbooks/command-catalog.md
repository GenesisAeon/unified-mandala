...
| `pnpm ci:verify`                                                                                                 | `pnpm`       | `node scripts/ci-verify.mjs`                                                                         | Runs the CI aggregator (Typecheck, Unit, Coverage, Schema/Map/Sanity, Policy) used by `ci-verify.yml`. |
...
| `pnpm -F @unified-mandala/ai dev`  | `pnpm` | `pnpm -F @unified-mandala/ai dev`                                                | Sends a sample prompt through the @unified-mandala/ai Responses wrapper (requires `OPENAI_API_KEY`).                                                                                                                  |
| `pnpm -F @unified-mandala/ai nats` | `pnpm` | `pnpm -F @unified-mandala/ai nats`                                               | Starts the optional NATS request/reply worker on `ai.request` for cross-service AI calls.                                                                                                                             |
| `pnpm -F @unified-mandala/api dev` | `pnpm` | `pnpm -F @unified-mandala/api dev`                                               | Launches the Express API exposing `/api/ai/chat`; set `AI_TRANSPORT=nats` to relay via the worker.                                                                                                                    |
