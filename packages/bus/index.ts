...
export function bridgeRedis(sub: Redis, pub: Redis, channel = 'um:bus') {
  ...
  sub.on('message', (_ch, msg) => {
    try {
      ...
      bus.emit(e.type as any, e.data);
    } catch {}
  });
  bus.on('agent.event', (data) =>
    pub.publish(channel, JSON.stringify({ type: 'agent.event', data })),
  );
  bus.on('kpi.update', (data) =>
    pub.publish(channel, JSON.stringify({ type: 'kpi.update', data })),
  );
  return () => {
