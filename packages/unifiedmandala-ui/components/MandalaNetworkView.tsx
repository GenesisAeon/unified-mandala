...
const colorForCREP = ({ C, R, E }: MandalaNode['crep']) => getCREPPhaseColor({ C, R, E });
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <button
        ...
        className="mt-4 p-2 bg-blue-600 text-white rounded-lg shadow"
      >
        Export as SVG
      </button>
      {selectedNode ? (
        <div data-testid="node-details" className="mt-4 p-2 border rounded text-center">
          ...
          <div>
            C:{selectedNode.crep.C} R:{selectedNode.crep.R} E:{selectedNode.crep.E} P:
            {selectedNode.crep.P}
          </div>
          {selectedNode.poetry && <div className="italic">{selectedNode.poetry}</div>}
        </div>
      ) : (
        <div data-testid="node-details" className="mt-4 text-sm text-gray-600">
          No node selected
        </div>
      )}
