  ...
  return (
    <div style={{ display: 'grid', gap: 8 }}>
      ...
      <div style={{ display: 'grid', gap: 6 }}>
        {filtered.map((d) => (
          <div key={d.id} style={{ padding: 8, border: '1px solid #333', borderRadius: 8 }}>
            <div
              style={{
                display: 'flex',
            ...
            {d.summary && <div style={{ opacity: 0.85, marginTop: 4 }}>{d.summary}</div>}
            <div
              style={{
                marginTop: 6,
                display: 'flex',
            ...
            {d.path && <div style={{ marginTop: 6, fontSize: 12, opacity: 0.7 }}>{d.path}</div>}
          </div>
        ))}
        {!filtered.length && <div style={{ opacity: 0.7 }}>Keine Einträge.</div>}
      </div>
    </div>
  );
}
